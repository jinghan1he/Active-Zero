...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        if indices_file:
            if os.path.exists(indices_file):
                ...
                print(f"Selected {len(indices)} indices from {indices_file}")
                self.dataset = self.dataset.select(indices)
            else:
                assert num_samples > 0, "num_samples must be greater than 0"
                # indices = list(range(num_samples))
                indices = random.sample(list(range(len(self.dataset))), num_samples)
                np.save(indices_file, np.array(indices))
                print(f"Selected {len(indices)} indices and saved to {indices_file}")
                self.dataset = self.dataset.select(indices)
    ...
    def _build_messages(self, example: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        if self.format_prompt:
            ...
            if "<image>" in self.format_prompt:
